...
- Specified left-to-right order for `using`-marked assignments across all three decomposition kinds

- Added clang implementation and implementation-tracker references
...
**`operator=`** The hidden variable _e_ is introduced exactly as today (per [[dcl.struct.bind] ¶1](https://eel.is/c++draft/dcl.struct.bind)). The difference lies in how each element SBᵢ is resolved: for a `using`-marked element, instead of introducing a new name, the implementation assigns from the corresponding element of _e_ to the existing variable via `operator=`.

**Order of assignment.** For all three decomposition kinds (tuple-like, array, and otherwise/aggregate), the elements SB₁, …, SBₙ are resolved in that lexical order: SBᵢ — its assignment if `using`-marked, its reference-binding otherwise — is fully completed before SBᵢ₊₁ is evaluated. In particular, for a `using`-marked SBᵢ, the assignment reads its right-hand element from _e_ and writes it via `operator=` before any later SBⱼ (j > i) is evaluated.

*Rationale:* tuple-like decomposition already has this ordering "for free" from [[dcl.struct.bind] ¶7](https://eel.is/c++draft/dcl.struct.bind#7), since each SBᵢ there is a separately-initialized variable, sequenced by ordinary declaration semantics. Array and aggregate decomposition never needed an order, since without `using` every SBᵢ is a side-effect-free reference binding. `using` adds `operator=` side effects, making the order observable: `auto& [_, using arr[0], using arr[1]] = arr;` intends an evict-and-shift, but silently corrupts `arr` under the other order. This rule generalizes the existing tuple-like guarantee to the two forms that previously had none.
...
- [P3817 implementation (Compiler Explorer)](https://godbolt.org/z/PPssMbf4E) — experimental Clang build (`clang_p3817`)
- [P3817 implementation tracker](https://github.com/regevran/llvm-project-fork/blob/p3817/README-P3817.md)
- [Active work on P3817 (GitHub issues)](https://github.com/yonisimian/cpp-proposals/issues)
